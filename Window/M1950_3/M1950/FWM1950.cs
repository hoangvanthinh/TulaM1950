...
namespace M1950
{
    public partial class CFW : Form
    {
        ...
        private void Read_result_ChecksumMCU()
        {
            ...
            {
                MessageBox.Show("Checksum < 8DG!");
            }
            textBox1.Text = temp;
            Checksum_Result.Text = temp;

            // phan loai loi
            if(Checksum_Result.Text == Checksum_Ans.Text)
            {
                Checksum_Result.BackColor = Color.Lime;
                for(int i = 0; i < 8;i++)
                {
                    if(M1950_RS232.SOCKET_Status[i] == STATUS_OK_Command)
                    {
                        M1950_RS232.SOCKET_Status[i] = STATUS_OK_Checksum;
                        List_SOCKET[i].BackColor = Color.LimeGreen;
            ...
            {
                Checksum_Result.BackColor = Color.Red;
                for (int i = 0; i < 8; i++)
                {
                    if (M1950_RS232.SOCKET_Status[i] == STATUS_OK_Command)
                    {
                        M1950_RS232.SOCKET_Status[i] = STATUS_NG_Checksum;
...
///===============================================================================================================
        ...
        void Logfiletxt()
        {
            ...
            {
                int j = i + 1;
                if(M1950_RS232.SOCKET_Status[i] == STATUS_NG_Command) // NG
                {
                    ...
                    {
                        writer.WriteLine("SOCKET" + j + "___" + comboBox_user.SelectedItem.ToString()  + "_" + DateTime.Now.ToString("MM-dd-yyyy-HH:mm:ss") + "_NG");

                    }
                }
                if (M1950_RS232.SOCKET_Status[i] == STATUS_NG_Checksum) // NG checksum
                {
                    string Full_path_NG_file = Path.Combine(Full_path_NG, "[SOCKET " + j + "_" + comboBox_user.SelectedItem.ToString() + "] W" + time + ".txt");
                    using (FileStream fileStream = File.Create(Full_path_NG_file))
                    using (StreamWriter writer = new StreamWriter(fileStream))
                    {
                        writer.WriteLine("SOCKET" + j + "___" + comboBox_user.SelectedItem.ToString() + "_" + Checksum_Result.Text + "_" + DateTime.Now.ToString("MM-dd-yyyy-HH:mm:ss") + "_NG");

                    }
                }
                else if (M1950_RS232.SOCKET_Status[i] == STATUS_OK_Command) //OK
                {
                    ...
                    using (StreamWriter writer = new StreamWriter(fileStream))
                    {
                        writer.WriteLine("SOCKET" + j + "___" + comboBox_user.SelectedItem.ToString() + "_" + Checksum_Result.Text + "_" + DateTime.Now.ToString("MM-dd-yyyy-HH:mm:ss") + "_OK");
        ...
        private void Remote_Command(object sender, EventArgs e)
        {
            if (Product_code.SelectedItem != null)
            {
                ...
                Checksum_Result.BackColor = Color.White;
                Command.Text = null;
